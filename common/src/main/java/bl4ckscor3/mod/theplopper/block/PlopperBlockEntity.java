...
public class PlopperBlockEntity extends BaseContainerBlockEntity {
	...
	private NonNullList<ItemStack> inventory = NonNullList.withSize(STORAGE_SIZE_WITH_UPGRADE, ItemStack.EMPTY);
	...
	/**
	 * Handles the given item stack maybe having been added to the plopper
	 *
	 * @param ie The ItemEntity that gets sucked up
	 * @param inserted How many items got inserted
	 * @param toInsert How many items should have been inserted
	 * @param stack The stack that is inserted
	 * @return true if (part of) the stack has been sucked up, false if the stack couldn't be sucked up
	 */
	public boolean handleSucking(ItemEntity ie, int inserted, int toInsert, ItemStack stack) {
	...
	public AbstractContainerMenu createMenu(int windowId, Inventory playerInv) {
		return new PlopperMenu(windowId, playerInv, getBlockPos());
	}
	...
	public boolean canPlaceItem(int slot, ItemStack stack) {
		return slot != UPGRADE_SLOT || stack.is(ThePlopper.THE_PLOPPER_ITEM.get()) && getItem(slot).getCount() + stack.getCount() <= 7;
	}
